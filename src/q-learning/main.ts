...
const showExampleButton = document.getElementById(
  "show-example",
...
const onChange = () => {
  ...
  if (rangeElement) {
    if (Number(rangeElement.value) > round.length) {
      ...
      if (rangeDisplayElement)
        rangeDisplayElement.innerHTML = String(round.length);
    }
  ...
  for (let i = 0; i < round.length; ++i) {
    ...
    if (line.length !== 100) {
      if (errElement)
        errElement.innerHTML = `${i + 1} 行目の要素数が正しくありません。 ${line.length} 個の値があります。 100 個の要素を空白区切りで指定してください。`;
      return;
    }
    for (let j = 0; j < line.length; ++j) {
      ...
      if (Number.isNaN(num)) {
        if (errElement)
          errElement.innerHTML = `${i + 1} 行目の ${j + 1} 番目の値 "${line[j]}" は数値ではありません。`;
        return;
      }
    }
  }
...
if (rangeElement) {
  rangeElement.addEventListener("input", () => {
    ...
    if (round < 0 || round >= values.length) {
      if (errElement)
        errElement.innerHTML = `0 以上 ${values.length - 1} 以下の値を指定してください。`;
      return;
    }
...
if (showExampleButton) {
  showExampleButton.addEventListener("click", async () => {
    const sample = await fetch("/univ-vis/qlearning-example.txt").then((res) =>
      res.text(),
    );
